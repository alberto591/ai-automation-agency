...
def enrich_with_ai(property_data: dict, ai_adapter: MistralAdapter) -> dict:
    ...
    fields_to_fix = [
        f for f in [
            'sqm', 'rooms', 'bathrooms', 'energy_class', 'floor', 
            'has_terrace', 'has_garden', 'has_parking', 'has_air_conditioning',
            'has_elevator', 'condition', 'heating_type'
...
def main():
    ...
    parser.add_argument("--index", type=int, default=0, help="Index of the property to use (from list).")
    parser.add_argument("--output", type=str, default="temp/property_brochure.pdf", help="Output file path.")
    parser.add_argument("--ai", action="store_true", help="Use AI to enrich missing fields.")
    parser.add_argument("--save", action="store_true", help="Save AI-enriched data back to Supabase.")
    parser.add_argument("--image", type=str, help="Force a specific image path/URL (useful for demos).")
