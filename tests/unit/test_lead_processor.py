...
def test_process_lead_success(processor, mock_db, mock_ai, mock_msg):
    ...
    from application.workflows.agents import IntentExtraction, PropertyPreferences, SentimentAnalysis
    mock_extraction = IntentExtraction(budget=100000, intent="VISIT", entities=["Milano"])
    mock_prefs = PropertyPreferences(rooms=[3], zones=["Milano"], features=["balcony"], property_types=["apartment"])
    ...
    # Mock other ports
    mock_db.get_properties.return_value = [{"title": "Test Prop", "price": 100000, "similarity": 0.9}]
    mock_db.get_cached_response.return_value = None
    mock_db.get_lead.return_value = {"customer_phone": "+39333000000", "customer_name": "Test User", "is_ai_active": True, "messages": []}
