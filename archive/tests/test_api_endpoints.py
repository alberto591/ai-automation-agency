...
def test_post_leads_valuation_trigger(client, mock_external_services):
    ...
    supabase.table.return_value.select.return_value.ilike.return_value.execute.return_value = (
        MagicMock(data=[{"price_per_mq": 4000}])
    )
...
def test_portal_webhook_immobiliare(client, mock_external_services):
    ...
    response = client.post(
        "/webhooks/immobiliare", json=payload, headers={"x-webhook-key": "test_webhook_secret"}
    )
