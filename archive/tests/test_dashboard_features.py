...
def test_save_lead_to_dashboard_profiling(mock_supabase):
    ...
    """
    # Setup mock chain
    mock_execute = mock_supabase.table.return_value.select.return_value.eq.return_value.execute
...
def test_manual_message_endpoint(mock_send_safe):
    ...
    payload = {"phone": "+393331112222", "message": "Ciao, sono l'agente umano!"}
    ...
    update_calls = [
        c for c in mock_supabase.table.return_value.update.call_args_list if "messages" in c[0][0]
    ]
